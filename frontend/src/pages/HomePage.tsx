...
export const HomePage = () => {
  ...
  // Available categories for dropdown; update as needed
  const categories = [
    "All Categories",
    "Clothing",
    "Furniture",
  ...
  useEffect(() => {
    fetchProducts();
    // Only fetch on mount and category/search changes
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const data: Product[] = await api.get(
        `/products?limit=${limit}&offset=${offset}${categoryParam}${searchParam}`
      );
      setProducts(data);
    } catch (error) {
      toast({
  ...
  const handleAddToCart = async (productId: string) => {
    ...
    try {
      await api.post("/cart/", { product_id: productId }); // call backend!
      toast({
        title: "Added to Cart",
      ...
      <section className="relative h-96 bg-gradient-to-r from-eco-green to-eco-light overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 h-full flex items-center">
          <div className="max-w-2xl text-primary-foreground">
            <h1 className="text-5xl font-bold mb-6">Discover Sustainable Treasures</h1>
            ...
            <div className="flex gap-4">
              ...
              <Button
                variant="outline"
                className="text-primary-foreground border-primary-foreground hover:bg-primary-foreground hover:text-eco-green"
